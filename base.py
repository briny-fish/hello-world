...
def pre_process():
    ...
    cd= CountVectorizer(ngram_range=(1,2),analyzer='char')
    discuss_tf = tf.fit_transform(data['cut_comment'])
    discuss_cd=cd.fit_transform(data['cut_comment'])
    tmp=pk.load(open('bdc.sav', 'rb'))
    print(discuss_cd[0])
    tt=list()
    for i in range(discuss_cd.shape[0]):
        tt.append(np.dot(discuss_cd[i],tmp))
    print(tt[0])
    #tmp = Bdc.cal_bdc(discuss_cd[:nrw_train].toarray(),y1,10)

   # print(discuss_tf.toarray())
